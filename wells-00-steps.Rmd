...
```{r}
for (i in 1:nrow(mw_2)) {
    print(mw_2[i, 4][[1]]@scalars)
}
```

### extract the scalars dataframes

```{r}
do.call(rbind, lapply(1:nrow(mw_2), function(x) mw_2[x, 4][[1]]@scalars))
```
...
setMethod("initialize", "MultiWell_4", function(.Object, aList, ...) {
    # the column index for the well_object
    .Object@wo_col <- 4L
    ...
    # .Object@well_object_column <- which(grepl("well_object", colnames(mw3)))
    # .Object@wo_col <- 4
...
setMethod("show", "MultiWell_4", function(object) {
    data_part <- getDataPart(object)                           # get the data
    rownames(data_part) <- as.character(slot(object, "well_names"))  # well name as row name
    
    # the well object is column #4
    # add start ro well object
    colnames(data_part)[4] <- paste(colnames(data_part)[4], "*", sep = "")  
    cat("MultiWell \n")   # table name
    
    # pretty printing, justify right, remove quotes
    print(data_part, right = TRUE, quote = FALSE)
    # print(data)
})

setGeneric("summaryScalars", function(object) standardGeneric("summaryScalars"))
setMethod("summaryScalars", "MultiWell_4", function(object) {
    data <- getDataPart(object) 

    # get a dataframe of all the scalars
    # using c() for extra arguments make the strange characters dissapear
    # the extra argument is stringsAsFactors
    scalars <- do.call(rbind, c(
        lapply(1:nrow(data), function(x) data[x, object@wo_col][[1]]@scalars), 
...
setGeneric("summaryWellTests", function(object) standardGeneric("summaryWellTests"))
setMethod("summaryWellTests", "MultiWell_4", function(object) {
    data <- getDataPart(object) 
    ...
    scalars %>% 
      select(well, wt_count, wt_enabled, wt_disabled)
}) 

mw4 <- new("MultiWell_4", multiwell)
summaryWellTests(mw4)
```
