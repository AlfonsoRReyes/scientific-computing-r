...
```{r} 
bisection <- function(f, interval, tol=.Machine$double.eps^0.25,
maxiter=1000) { 
a <- interval[1] 
b <- interval[2] 
if (a >= b) stop("startpoint a must be smaller than endpoint b") 
if (f(a)*f(b) >= 0) stop("f(a) and f(b) must
have a different sign") 
i <- 1 
while (i <= maxiter) #limit iterations to prevent infinite loop 
    { mid <- (a + b)/2 # new midpoint estim.prec <- (b - a)/2 # estimated precision 
    if (f(mid) == 0 | estim.prec < tol) # root found { if
(f(mid) == 0) 
    estim.prec <- 0 
