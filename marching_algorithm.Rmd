...
# rows has to be greater than segments to allocate the zero
...
for (i in seq_len(n)) {
  if (i == 1) {
      depth.prev = depth.top
  } else {
      depth.prev = depths[i-1]
    }
  dL = depths[i] - depth.prev
  p1 = p0 + dp.dz * dL
  eps  = 1
  iter = 1
  ...
  #cat(, "\n")
  #cat(sprintf("%2d %8.1f %8.1f %2d %8.3f \n", i, p0, p1, iter, dp.dz))
  p0 = p1
