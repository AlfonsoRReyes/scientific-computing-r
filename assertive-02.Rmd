...
mad <- function (x, center = median(x), constant = 1.4826, na.rm = FALSE, 
    ...
    assert_is_numeric(x)
    assert_is_numeric(constant)
    assert_is_of_length(constant, 1)
    assert_is_logical(na.rm)
    if (na.rm) 
        x <- x[!is.na(x)]
    n <- length(x)
...
mad(c(1:9))
mad(c(1:9), constant = 1)
mad(c(1:8, 100), constant = 1)
# mad(c(1,2,"a"))
# Error in mad(c(1, 2, "a")) : is_numeric : x is not of class 'numeric'; it has class 'character'.

```

```{r}
mad(c(1:8), constant = c(1.5, 2))
```
