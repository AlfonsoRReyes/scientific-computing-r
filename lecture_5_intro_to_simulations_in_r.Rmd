...
hist(samplemeans,main=paste("Histogram of samplemeans for n
 =",n),col="hotpink",cex.main=1.5)
```
...
lower.corrected=p.hats-(qnorm(0.975)*sqrt(p.hats*(1-p.hats)/n)+0.5/n)
upper.corrected=p.hats+(qnorm(0.975)*sqrt(p.hats*(1-p.hats)/n)+0.5/n)
...
lower.corrected=p.hats-(qnorm(0.975)*sqrt(p.hats*(1-p.hats)/n)+0.5/n)
upper.corrected=p.hats+(qnorm(0.975)*sqrt(p.hats*(1-p.hats)/n)+0.5/n)
cover.prob.regular[i]=mean(lower.regular<=p & upper.regular>=p)
cover.prob.corrected[i]=mean(lower.corrected<=p & upper.corrected>=p)
}
sum(cover.prob.corrected>=0.95)
sum(cover.prob.regular>=0.95)

boxplot(cover.prob.regular,cover.prob.corrected,
names=c("Regular","Corrected"),main="Simulated
CI Coverage Probabilities",
cex=1.5,cex.axis=1.5,col="gray")
abline(a=0.95,b=0,lty=2)
plot(cover.prob.regular~p.vector,col="red",
type="l",lwd=2,main="Simulated CI Coverage
Probabilities \n at Various Levels of p")
lines(cover.prob.corrected~p.vector,col="blue",
type="l",lwd=2)
legend(x=0.3,y=0.5,legend=c("Regular","Corrected")
