...
## 
We demonstrate in this analysis that the fastest methods to create row by row in a data frame are:
...
Different methods are tested with different size of rows.
...
for(i in seq_len(length(timeSeq))) {
  nRow <- timeSeq[[i]]
  ti <- microbenchmark(mkFrameInPlace(nRow, nCol),
                       mkFrameDataTableFor(nRow, nCol),
    times=10)
  ti <- data.frame(ti,
                   stringsAsFactors=FALSE)
...
mkFrameDplyrBind <- function(nRow,nCol) {
 dplyr::bind_rows(replicate(nRow, 
                     data.frame(mkRow(nCol), stringsAsFactors=FALSE), 
                     simplify=FALSE))
}
...
mkFramePlyrLdply <- function(nRow,nCol) {
  plyr::ldply(seq_len(nRow),
              function(i) data.frame(mkRow(nCol), stringsAsFactors=FALSE))
}
...
mkFrameSQLite <- function(nRow, nCol) {
  con <- RSQLite::dbConnect(RSQLite::SQLite(), ":memory:")
  ri <- mkRow(nCol)
  RSQLite::dbWriteTable(con, 't', as.data.frame(ri))
  for(i in seq_len(nRow)) { 
    ri <- mkRow(nCol)
    RSQLite::dbWriteTable(con, 't', as.data.frame(ri), append = TRUE)
    }
}
```
...
```{r}
#nRow = 10; nCol = 10
mkFrameWithList <- function(nRow, nCol) {
  result <- data.frame()
  for (i in seq_len(nRow)) {
    ri <- mkRow(nCol)
    ri <- data.frame(ri)
...
for(i in seq_len(length(timeSeq))) {
  nRow <- timeSeq[[i]]
  ti <- microbenchmark(mkFrameDplyrBind(nRow, nCol), 
                       ...
                       mkFrameEnvDataTable(nRow, nCol), 
                       mkFrameDataTableLapply(nRow, nCol),
                       mkFrameSQLite(nRow, nCol), 
                       mkFrameWithList(nRow, nCol),
...
## Tests excluding slowest
```{r}
library(dplyr)

timingsFaster <- timings %>%
  #mutate(exp)
  filter(expr != "mkFrameForLoop(nRow, nCol)") %>%
  filter(expr != "mkFramePlyrLdply(nRow, nCol)") %>%
  filter(expr != "mkFrameList(nRow, nCol)") %>%
  filter(expr != "mkFrameForList(nRow, nCol)") %>%
  filter(expr != "mkFrameDplyrBind(nRow, nCol)") %>%
  filter(expr != "mkFrameDaccum(nRow, nCol)") %>%
  filter(expr != "mkFrameSQLite(nRow, nCol)") 

print(plotTimings(timingsFaster))
  
```

## Test only the fastest

```{r}
library(dplyr)

timingsFastest <- timings %>%
  filter(
        expr == "mkFrameDataTableLapply(nRow, nCol)" | 
        expr == "mkFrameDataTableFor(nRow, nCol)" |
        expr == "mkFrameEnvDataTable(nRow, nCol)" |
        expr == "mkFrameInPlace(nRow, nCol)" |
        expr == "mkFrameVector(nRow, nCol)"
        
      )
  
  print(plotTimings(timingsFastest))
```
